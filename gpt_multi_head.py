...
class Head(nn.Module):
    ...
    def forward(self, x):
        B,T,C = x.shape
        ## YOUR CODE HERE
        # Applique les transformations de Key, Query et Value
        k = self.key(x)  # (B, T, head_size)
        q = self.query(x)  # (B, T, head_size)
        v = self.value(x)  # (B, T, head_size)
        
        # Calcul du produit de Q et K pour obtenir les poids d'attention
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, n_embd):
       ...
       # num_heads = n_head 
        ...
        self.proj = nn.Linear(n_embd, n_embd)  # projection finale 
        self.dropout = nn.Dropout(dropout)
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        # Using Sequential to stack 3 Blocks
        self.blocks = nn.Sequential(
            Block(n_embd, n_head=4),  # First block
            Block(n_embd, n_head=4),  # Second block
            Block(n_embd, n_head=4)   # Third block
        )
        # Language model head
